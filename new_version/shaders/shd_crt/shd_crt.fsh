...
void borderEffects( inout vec4 color, in vec2 uv)
{
	...
	if (borderMask < 0.9) {
		...
		borderColor += texture2D( gm_BaseTexture, clamp( uv + vec2( -texelSize.x, -texelSize.y), 0.01, 0.99));
		// Cardinals
		borderColor += texture2D( gm_BaseTexture, clamp( uv + vec2( 0.0, texelSize.y), 0.01, 0.99));
		borderColor += texture2D( gm_BaseTexture, clamp( uv + vec2( 0.0, -texelSize.y), 0.01, 0.99));
		borderColor += texture2D( gm_BaseTexture, clamp( uv + vec2( texelSize.x, 0.0), 0.01, 0.99));
		borderColor += texture2D( gm_BaseTexture, clamp( uv + vec2( -texelSize.x, 0.0), 0.01, 0.99));
		// Reduce the values back down to 0-1
		borderColor *= 0.111111111;
	}
