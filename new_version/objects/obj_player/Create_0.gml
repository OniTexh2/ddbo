...
// AFK Timers
afk_maxtime = 300 // In seconds
afk_maxtime *= fps // Frames to seconds
...
choose_direction = function (){
	if (dir == 0)		 sprite_index = (!running ? right : run_right);
	else if (dir == 90)  sprite_index = (!running ? back : run_back);
	else if (dir == 180) sprite_index = (!running ? left : run_left);
	else if (dir == 270) sprite_index = (!running ? forward : run_forward);	
}
...
state.add("normal", {
	enter: function(){
		...
		run_right = spr_player_runright;
		run_back = spr_player_runback;
		run_left = spr_player_runleft;
		run_forward = spr_player_runforward;
...
state.add("dead", {
	...
	step: function(){		
		if subimg >= 3 and !shot{
			shot = true
			audio_play_sound(snd_gunshot,0,0); // Sound
		}
		
		if subimg >= image_number-1 and animate{
			animate = false
			audio_play_sound(mus_after_death,0,1); // Music
		}
	}
});
...
state.add("walking", {
	...
	step: function(){
		...
		if(footstep_t = 0){
			audio_play_sound(snd_footstep_regular,0,0,1,0,sound_pitch);
...
state.add("unlock", {
	enter: function(){
		...
		audio_play_sound(snd_open_door, 0, 0, 1)
		audio_play_sound(snd_unlock, 0, 0, 1)
		
		instance_create_layer(x,y,"Instances",obj_lockdebris)
	},
	step: function(){		
		if subimg >= image_number - 1 and animate{
			state.change("normal");
...
state.add("shake", {
	enter: function(){
		...
		randomise()
		audio_play_sound(choose(snd_velvet_fyou1, snd_velvet_fyou2, snd_velvet_fyou3), 0, 0, 2)
	},
	step: function(){
