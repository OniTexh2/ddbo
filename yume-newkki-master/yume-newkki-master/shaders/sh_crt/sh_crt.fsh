...
#define ABERRATION_OFFSET vec2(0,0)
...
#define PULSE_INTENSITY 0.01
//Pulse width in pixels (times tau)
#define PULSE_WIDTH 30e1
//Pulse animation speed
#define PULSE_RATE 1e1
...
void main()
{
	...
	vec2 cell_offset = (vec2(0, fract(floor(coord.x)*0.5)))*1.0;
    
    //Pixel coordinates rounded to the nearest cell
    vec2 mask_coord = (coord * MASK_SIZE) / u_resolution;
    
    //color
	vec4 color = texture2D(gm_BaseTexture, mask_coord);
	
	mask_coord -= 0.5;
	mask_coord *= 0.995;
	mask_coord += 0.5;
	
	color.g = texture2D(gm_BaseTexture, mask_coord).g;
