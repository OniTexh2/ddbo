...
choose_direction = function (){
	...
	else if (dir == 270) sprite_index = front;	
}
...
state.add_child("normal","penguin", {
	enter: function(){	
		...
		front =	spr_player_penguin_front;
		wake_state = -1;
...
#region tv
//this is the default tv screen drawing function
tv_screen_static = function(){
	if(tv_value == 1){
...
tv_screen_on_off = function(){
	if(input_check_pressed("special")){
		if(tv_value == 1) tv_value = 0;
		else tv_value = 1;
	}		
}

//tv_screen_draw and tv_screen_toggle can be overridden in specific rooms,
//and reset to tv_screen_static and tv_screen_on_off again when leaving the rooms
tv_screen_draw = tv_screen_static;
tv_screen_toggle = tv_screen_on_off;
