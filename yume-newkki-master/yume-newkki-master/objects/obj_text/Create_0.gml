...
state.event_set_default_function("draw_gui", function() {
	...
	while(i <= string_length (message[@ message_current]) && i <= cutoff){
		...
		{
			var so = (t + i);
			var shift = sin((so*pi*freq)/game_speed)*amplitude;
		}
		else var shift = 0;
		...
		if (modifierColor == true) {
			switch textColor {	
				...
				case "rainbow":
				{
					var hue = ((((t+i)*255)/game_speed)*0.5)%255;
...
state.add("start", {
	...
	step: function(){
		height_ = ease_out(t++,0,height,game_speed/3);
...
state.add("stop", {
	...
	step: function(){
		height_ = ease_out(t++,height_,-height,game_speed/3);
